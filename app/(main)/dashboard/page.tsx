  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 border-b border-[#A9A9A9]/20 pb-8">
        <div>
          <h1 className="text-4xl font-black text-[#1A1A1A] tracking-tight">{t("title")}</h1>
          <p className="text-[#A9A9A9] font-medium mt-2">{t("subtitle")}</p>
        </div>
        <LogoutButton />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl border border-[#A9A9A9] shadow-sm overflow-hidden">
          <div className="bg-[#1A1A1A] px-8 py-4">
            <h2 className="text-lg font-bold text-white">{t("profileDetails")}</h2>
          </div>
          <div className="p-8 space-y-6">
            <div className="space-y-1">
              <label className="text-xs font-black text-[#A9A9A9] uppercase tracking-wider">{t("emailAddress")}</label>
              <p className="text-lg font-bold text-[#1A1A1A]">{session.user.email}</p>
            </div>

            <div className="flex gap-12">
              <div className="space-y-1">
                <label className="text-xs font-black text-[#A9A9A9] uppercase tracking-wider">{t("accountRole")}</label>
                <p className="text-lg font-bold text-[#C8102E] capitalize">{(session.user.role || "user").toLowerCase()}</p>
              </div>
              <div className="space-y-1">
                <label className="text-xs font-black text-[#A9A9A9] uppercase tracking-wider">{t("userId")}</label>
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-red-50 border border-[#C8102E]/10 p-8 rounded-2xl flex-1 flex flex-col justify-center">
            <h3 className="text-xl font-black text-[#C8102E] mb-2">{t("welcomeBack")}</h3>
          ...
          >
            <span className="font-bold text-[#1A1A1A]">{t("viewMyOrders")}</span>
